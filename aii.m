...
% Περιορισμοί (saturation)
m_hat = min(max(m_hat, 1), 2);
b_hat = min(max(b_hat, 0), 0.5);
k_hat = min(max(k_hat, 0.5), 1);

% Ανακατασκευή κίνησης
x_hat = zeros(1, N); dx_hat = zeros(1, N); ddx_hat = zeros(1, N);
for i = 1:N-1
